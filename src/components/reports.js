...
export function reports() {
  ...
  if (action === "view") return `<section class="card"><div class="section-title"><h3>Saved report cards</h3><button class="btn ghost" onclick="navigateBack()">Back</button></div>${table("", ["Report", "Student", "Period", "Date"], state.reportCards.map(row => [row.id, getStudent(row.studentId)?.name, row.periodLabel, row.date]))}</section>`;
  if (action === "performance") return `<section class="card"><div class="section-title"><h3>Student Performance</h3><button class="btn ghost" onclick="navigateBack()">Back</button></div>${reportControls(report)}</section>${reportCardPrint(report)}`;
