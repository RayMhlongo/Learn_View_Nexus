import { average, badge, emptyState, formData, initials, scorePercent, spark, table } from "../utils.js";
...
export function students() {
  ...
  <div class="grid cols-3"><section class="card"><div class="section-title"><h3>Students</h3><button class="btn primary" onclick="openStudent()">Add</button></div><div class="timeline">${rows.map(studentListItem).join("") || emptyState("No students match these filters.")}</div></section><section class="card" style="grid-column:span 2">${profile(selected)}</section></div>`;
}
