...
export function reportCardPrint(report) {
  ...
  return `<section class="print-doc report-template only-printable">${watermark()}<div class="doc-content">
    ...
    <table class="print-table report-table"><thead><tr><th>Subject</th><th>Assessments</th><th>Average (%)</th><th>Grade</th><th>Comment</th></tr></thead><tbody>${rows.map(row => `<tr><td>${row[0]}</td><td>${row[2] || "No marks"}</td><td><strong class="strong-value">${row[3]}</strong></td><td>${grade(row[3])}</td><td>${row[4] || "Continued practice recommended."}</td></tr>`).join("")}</tbody></table>
